...
def find_color1(frame):
    ...
    hsv_lowerbound = np.array([34, 50, 64])  # replace THIS LINE w/ your hsv lowerb
    hsv_upperbound = np.array([60, 255, 255])  # replace THIS LINE w/ your hsv upperb
...
def find_color2(frame):
    ...
    """
    hsv_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    hsv_lowerbound = np.array([0, 170, 0])  # replace THIS LINE w/ your hsv lowerb
    hsv_upperbound = np.array([4, 255, 255])  # replace THIS LINE w/ your hsv upperb
    mask = cv2.inRange(hsv_frame, hsv_lowerbound, hsv_upperbound)
...
while (1):
    ...
    if found_color1 and found_color2:
        ...
        if color2_y < color1_y and color2_x > color1_x:
            angle_text = str(float(angle))
        elif color2_y < color1_y and color2_x < color1_x:
            angle_text = str(float(180 - angle))
        elif color2_y > color1_y and color2_x < color1_x:
            angle_text = str(float(180 + angle))
        elif color2_y > color1_y and color2_x > color1_x:
            angle_text = str(float(360 - angle))
